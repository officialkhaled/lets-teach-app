                ...
                <div class="col-md-12">
                    <section class="block block-rounded block-bordered block-link-shadow" style="padding: 30px;">
                        <header class="d-flex justify-content-between">
                            ...
                            <div>
                                <a href="{{
    										(currentUser()->hasRole('super-admin') || currentUser()->hasRole('admin')) ? route('admin.admin-dashboard') :
											(currentUser()->hasRole('tutor') ? route('tutor.tutor-dashboard') :
                        ...
                        <div class="row">
                            <form method="post" class="space-y-3" enctype="multipart/form-data"
                                  action="{{ (currentUser()->hasRole('super-admin') || currentUser()->hasRole('admin')) ? route('admin.profile.update') :
                                             (currentUser()->hasRole('tutor') ? route('tutor.profile.update') : route('student.profile.update'))
                                          }}"
                            >
                                @csrf
                                @method('patch')
